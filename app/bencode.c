...
unsigned char *encode_dict_bencode(bencode *bencoded_dict) {
  ...
  for (int temp = 0; temp < bdict->length; temp++) {
    ...
    buf[str_index++] = (char)key->raw_size;
    ...
    case BENCODE_STRING: {
      ...
      buf[str_index++] = (char)val->raw_size;
...
char *encode_list_bencode(bencode *lst) {
  ...
  while (length < blst->length) {
    ...
    case BENCODE_STRING: {
      ...
      snprintf(arr + str_index, val->raw_size, "%zu:%s", strlen(val->value),
               val->value);
      str_index += val->raw_size;
      break;
    }
    ...
    case BENCODE_LIST: {
      ...
      memcpy(arr + str_index, lst_element, blst->values[length]->raw_size);
    ...
    case BENCODE_DICTIONARY: {
      ...
      memcpy(arr + str_index, dict_element, blst->values[length]->raw_size);
      str_index += blst->values[length]->raw_size;
      break;
    }
    case BENCODE_INVALID: {
...
bencode *decode_dict_bencode(const char *bencoded_value) {
  ...
  while (curr - bencoded_value < strlen(bencoded_value) - 1) {
    ...
    bdict->keys = realloc(bdict->keys, (bdict->length + 1) * sizeof(*b_key));
...
void bencode_print_dict_from_key(bencode *b, int key_sum, const char *key,
  ...
  while (counter < bdict->length) {
    if (strcmp(bdict->keys[counter]->value, key) == 0) {
      const char *next_arg = va_arg(args, const char *);
      if (key_sum > 1) {
        bencode_print_dict_from_key(bdict->values[counter], --key_sum, next_arg,
                                    args);
        break;
      }
      ...
      case BENCODE_STRING: {
        printf("%s", ((bencode_string *)bdict->values[counter])->value);
        break;
      }
      case BENCODE_INTEGER: {
...
void compare_bencode_dicts(bencode_dictionary *first, bencode_dictionary *sec) {
  printf("[TYPE] %d : %d\n", first->type, sec->type);
